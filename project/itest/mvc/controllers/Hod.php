...
class Hod extends Admin_Controller {
...
*/
    ...
    {
        parent::__construct();
        $this->load->model("Hod_m");
        $this->load->model("document_m");
        $this->load->library('updatechecker');
        $language = $this->session->userdata('lang');
        $this->lang->load('hod', $language);
    ...
    public function send_mail_rules()
    {
        $rules = [
            ...
            [
                'field' => 'message',
                'label' => $this->lang->line("Hod_message"),
    ...
    public function photoupload()
    {
        ...
        $user = [];
        if ( (int) $id ) {
            $user = $this->Hod_m->get_teacher($id);
        }
    ...
    {
        $this->data['teachers'] = $this->Hod_m->get_teacher();
        $this->data["subview"]  = "hod/index";
        // print_r($this->data);
        // exit();
    ...
    public function add()
    {
        ...
        if ( $_POST ) {
            ...
            if ( $this->form_validation->run() == false ) {
                ...
                $array['password']        = $this->Hod_m->hash($this->input->post("password"));
                $array['usertypeID']      = 5;
    ...
    public function edit() {
		...
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$this->data['teacher'] = $this->Hod_m->get_teacher($id);
			if($this->data['teacher']) {
				if($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						$this->Hod_m->update_teacher($array, $id);
						$this->session->set_flashdata('success', $this->lang->line('menu_success'));
	...
	private function getView($teacherID) {
		if((int)$teacherID) {
			$teacherinfo = $this->Hod_m->get_teacher($teacherID);
	...
	public function lol_username() {
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$teacher_info = $this->Hod_m->get_teacher($id);
			$tables = array('student' => 'student', 'parents' => 'parents', 'teacher' => 'teacher', 'user' => 'user', 'systemadmin' => 'systemadmin');
			$array = array();
			$i = 0;
			foreach ($tables as $table) {
				$user = $this->Hod_m->get_username($table, array("username" => $this->input->post('username'), "username !=" => $teacher_info->username));
				if(inicompute($user)) {
			...
			$i = 0;
			foreach ($tables as $table) {
				$user = $this->Hod_m->get_username($table, array("username" => $this->input->post('username')));
				if(inicompute($user)) {
					$this->form_validation->set_message("lol_username", "%s already exists");
					$array['permition'][$i] = 'no';
				} else {
	...
	public function print_preview() {
		if(permissionChecker('teacher_view')) {
			$teacherID = htmlentities(escapeString($this->uri->segment(3)));
			if ((int)$teacherID) {
				$this->data["teacher"] = $this->Hod_m->get_teacher($teacherID);
	...
	public function unique_email() {
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$teacher_info = $this->Hod_m->get_single_teacher(array('teacherID' => $id));
			$tables = array('student' => 'student', 'parents' => 'parents', 'teacher' => 'teacher', 'user' => 'user', 'systemadmin' => 'systemadmin');
			$array = array();
			$i = 0;
			foreach ($tables as $table) {
				$user = $this->Hod_m->get_username($table, array("email" => $this->input->post('email'), 'username !=' => $teacher_info->username));
				if(inicompute($user)) {
			...
			$i = 0;
			foreach ($tables as $table) {
				$user = $this->Hod_m->get_username($table, array("email" => $this->input->post('email')));
				if(inicompute($user)) {
					$this->form_validation->set_message("unique_email", "%s already exists");
					$array['permition'][$i] = 'no';
				} else {
	...
	function active() {
		if(permissionChecker('teacher_edit')) {
			...
			if($id != '' && $status != '') {
				if((int)$id) {
					if($status == 'chacked') {
						$this->Hod_m->update_teacher(array('active' => 1), $id);
						echo 'Success';
					} elseif($status == 'unchacked') {
						$this->Hod_m->update_teacher(array('active' => 0), $id);
						echo 'Success';
					} else {
						echo "Error";
					}
