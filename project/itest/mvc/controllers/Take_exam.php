        ...
        if((int) $onlineExamID) {
            ...
            if(inicompute($online_exam)) {
                ...
                if($_POST) {
                    ...
                    //$explain=$questionsBank['3']->explanation
                             ...
                             // print_r($correctAnswer);  
                                ...
                                if($typeID == 3) {
                                    foreach ($options as $answerID => $answer) {
                                        ...
                                        $user_ans=explode(' ',$getAnswer);
                                         ///$hint=count($options);
                                        $results=array_intersect($fans,$user_ans);
                                          // print_r($result);
                                          // exit();
                                        $correct_Answer=count($results);
                                        $prcntg[] = ($correct_Answer/$hint)*100;
                                        
                                        // echo "you have got'".$prcntg."' marks";
                                       // $gg['']=array(array_sum($prcntg));
...
//die();
                    ...
                    if ($typeID == 3) {
                        // $sql = "INSERT INTO online_exam_user_status SET totalPercentage = '".$prcntg."'";
                        // $this->db->insert('online_exam_user_status',$sql);
//print_r (array_sum($prcntg));   die(); 
                        //$pr = strip_tags($prcntg);
                        //print_r($pr);die();

                         //print_r($this->data['onlineExam']->onlineExamID);

                         //print_r($totalAnswer);
                         // print_r($userID);
                        // print_r($examTimeCounter);
                        $pp=array_sum($prcntg);
                        //print_r(($pp/$totalQuestionMark)*100);
                        $totalPercentages=($pp/$totalQuestionMark)*100;
                        //print_r($totalCorrectMark);
                        // print_r($totalQuestionMark);
                         //exit();
                    ...
                    $this->data['pp'] = $pp;
                    $this->data['totalPercentages'] = $totalPercentages;
                                                 $this->data["subview"] = "online_exam/take_exam/obj_result";
